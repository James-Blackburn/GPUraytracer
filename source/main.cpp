...
void writeToPPM(const char* filename, cl_float3* data, int width, int height, int maxVal) {
	...
	for (int row = 0; row < height; row++) {
		for (int col = 0; col < width; col++) {
			image[(row * width + col) * 3] = 255.999f * min(data[row * width + col].x, 1.0f);
			image[(row * width + col) * 3 + 1] = 255.999f * min(data[row * width + col].y, 1.0f);
			image[(row * width + col) * 3 + 2] = 255.999f * min(data[row * width + col].z, 1.0f);
		}
	}
